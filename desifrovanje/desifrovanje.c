...
/*Ovaj C program transformise sifrovanu zip arhivu u nesifrovanu zip arhivu.
/*Ulaz u program DESIFROVANJE je sifrovana (i kompresovana) zip arhiva koja se dobija koriscenjem 
/*programa PKZIP 2.04 sa ukljucenim sifrovanjem, kao i lozinka koja se koristi za desifrovanje
/*Program se iz komandne linije poziva sa parametrima sifr_arhiva nesifr_arhiva sifra,
/*gde sifr_arhiva predstavlja ime arhive koja ce se desifrovati (obavezno sa ekstenzijom .zip), 
/*desifr_arhiva je fajl koji se dobija kao izlaz iz programa, a sifra je lozinka koja ce se 
/*koristiti za desifrovanje. Izlaz iz programa DESIFROVANJE bi trebalo da bude desifrovana (ali kopmresovana)
/*zip arhiva koja ce moci da se  otvori pomocu pkunzip desifr_arhiva file.
/*Program DESIFROVANJE radi samo za jedan ulazni file.*/
...
int main(int argc, char **argv)
{
	...
	char *sifra;						/*lozinka pomocu koje ce se vrsiti desifrovanje*/

	unsigned char c;					/*karakter koji ce sluziti za prepisivanje podataka iz jedne arhive u drugu*/
	unsigned int crc;					/*crc32 value za dati fajl*/
	...
	sifra=argv[3];					/*Iz komandne linije se ucitava lozinka pomocu koje ce se vrsiti desifrovanje*/

	ulaz_file=fopen(argv[1], "rb");	/*Otvara se fajl koji ce se desifrovati; otvara se samo za citanje*/
	...
	/*U local file header se modifikuje polje Flags. Bit 0, koja oznacava da li ima/nema sifrovanja se sa 1 menja na 0.*/
	/*U local file header se vrednost polja Compressed size smanjuje za 12, zbog uklanjanja 11 random bajtova*/ 
	...
	flags=flags&0xfffe;				/*Bit 00 se postavlja na 0, kao pokazatelj da fajl nije sifrovan*/
	...
	file_size=file_size-12;			/*Velicina kompresovanog fajla ce se smanjiti zbog*/
	ispis4(izlaz_file, file_size);	/*uklanjanja encryption headera (12 bajtova)*/
	...
	/*Cita se encryption header: odbacuje se 11 random generisanih bajtova...*/
	for(i=0; i<11; i++)
		{
			fscanf(ulaz_file, "%c", &c);
			c=(unsigned char)(c&0xff);		/*Cita se jedan po jedan random bajt...*/
			c=c^key[3];						/*...desifruje se...*/
			update_keys(key,c);				/*...azurira se vrednost key[0]-key[2]...*/
		}

	/*...a zatim se cita 12. bajt koji predstavlja MSB(Crc32 value for file) i desifruje se. Njega uporedjujemo sa 
	/*crc vrednoscu dobijamom iz local file headera-a. Ako se ne podudaraju to je dokaz da je upotrebljena 
	/*lozinka pogresna. Taj bajt takodje ne upisujemo u izlaznu arhivu*/
	fscanf(ulaz_file, "%c", &c);
	c=c^key[3];
	if(c!=(unsigned char)((crc>>24)&0xff))
		{
	...
	/*Ucitava se jedan po jedan karakter, desifruje se i upisuje u zip_arhivu*/
	for(i=(file_size); i>0; i--)
		{
			fscanf(ulaz_file, "%c", &c);	/*Ucitava se jedan po jedan karakter iz ulazne arhive...*/
			c=c^key[3];						/*...desifruje se...*/
			update_keys(key, c);			/*...azurira se vrednost key0-key3...*/
			fprintf(izlaz_file, "%c", c);	/*...i upisuje se karakter u desifrovanu_arhivu*/
		}
	...
	/*Prvih 6 polja (ukupne duzine 16 bajtova) su ista pa se samo prepisuju*/
	for(i=16;i>0;i--)
		{
			fscanf(ulaz_file, "%c", &c);
			fprintf(izlaz_file, "%c", c);
	...
	/*Polje offset of cd wrt to starting disk se smanjuje za 12 zbog uklanjanja 12 random bajtova*/
	ofset = ucitaj4(ulaz_file);
	ofset=ofset-12;
	ispis4(izlaz_file, ofset);
...
unsigned int crc32(unsigned int prev, char c)	/*Funkcija za izracunavanje crc32 vrednosti koja se koristi*/
